...
export async function POST(request:Request){
  ...
  const deposit=Math.ceil(b.total_amount*.7),target=b.status==='pending'?deposit:b.total_amount,balance=Math.max(0,target-paid);if(!balance)return NextResponse.json({ok:false,error:'Required payment is already complete'},{status:409});
