...
export async function POST(request:Request){
  ...
  const deposit=Math.ceil(b.total_amount*.7);const balance=Math.max(0,deposit-paid); if(!balance)return NextResponse.json({ok:false,error:'Booking deposit is already paid'},{status:409});
